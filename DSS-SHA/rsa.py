...
class rsa:
    def __init__(self,x=0,weak=0,bits=1024):    
                    ...
                    if disc>=0:
                        ...
                        if sqrdt.to_integral()==sqrdt:
                            ...
                            if x.to_integral()==x and y.to_integral()==y:
                                if x*y==public[1]:      
                #if the roots are integers and they are factors of "n" return
    ...
    def generate_keys(self,weak):
        ...
        if (weak==0):       #private key "Strong" Mode
            print "\t[Weak mode OFF]"
            self.e=65537
            x=self.eea(self.e,euler)
            self.d=x[0]%euler
        else:               #private key "Weak" Mode
            print "\t[Weak mode ON]"
            ok=0
            nr=0
